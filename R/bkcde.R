...
pdf.kernel.bk <- function(x,X,h,a=-Inf,b=Inf) {
  ...
  z <- (x-X)/h
  (exp(-0.5 * z^2) * 0.3989422804014326779) / (h*(pnorm((b-x)/h)-pnorm((a-x)/h)))
}
...
bkcde.optim.fn <- function(h=NULL,
  ...
  if(cv.penalty.method=="nonneg" && degree>0) {
    ...
    ## If any of the sample estimates are negative, return a heavy penalty
    if(any(f < 0)) return(-sqrt(.Machine$double.xmax))
  }
  ...
  ## ls-cv function.
  if(degree==0) {
    ## The degree=0 estimator is always proper, so no need for if(proper.cv)
    ## here...
    ...
    if(proper.cv) {
      ...
      ## can exploit the multivariate Y capabilities of .lm.fit() when
  ...
  if(bwmethod=="cv.ml") {
    ...
    ## the multivariate Y capabilities of .lm.fit() when evaluating the
